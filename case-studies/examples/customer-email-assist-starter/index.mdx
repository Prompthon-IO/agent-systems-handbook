...
- [Gmail OAuth Setup](/workshops/gmail-oauth-setup)
...
For the direct OAuth auto-send path, follow the starter README:

```text
case-studies/examples/customer-email-assist-starter/README.md
```

For a click-through Google Cloud Console guide, use
[Gmail OAuth Setup](/workshops/gmail-oauth-setup).

In connector-first mode, dashboard approval stores issues as
`approved_to_send` for the Codex Gmail connector runner. If the advanced local
OAuth variables are configured, the dashboard can execute the deterministic
send path immediately after the undo countdown.
...
- `README.md`: direct Gmail OAuth setup, refresh-token guidance, and optional
  runtime variables
...
- Direct Gmail OAuth exists only as an advanced adapter for standalone local
  runs. Its required values are `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and
  `GOOGLE_REFRESH_TOKEN`; `CUSTOMER_EMAIL_ASSIST_OPERATOR_EMAIL` is optional in
  code but recommended for duplicate-send protection.
