...
每个表单也包含 `Skill Compass score proposal` 区块，并预填 Vision、Harness 和 Craft points。如果这个 issue 比默认拆分更偏发现与判断、工作流执行，或最终产物打磨，请修改这些数值。数值会显示在创建后的 issue 正文中，协作者也可以在批准 issue 前调整它们。
