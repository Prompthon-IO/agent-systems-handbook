...
每个表单都会询问 Prompthon repository track 和 work kind。仓库自动化会把这些答案转换为安全的 `track:*`、`kind:*` 和 `status: pending-review` 标签，因此贡献者不需要标签权限。repository track 用于路径策略和分流；它和 Skill Compass points 的拆分是两件事。

每个表单也包含 `Skill Compass score proposal` 区块，并预选 Vision、Harness 和 Craft point 下拉值。如果这个 issue 比默认拆分更偏发现与判断、工作流执行，或最终产物打磨，请在 0 到 100 之间按 5 分步长选择数值。数值会显示在创建后的 issue 正文中，协作者也可以在批准 issue 前调整它们。
