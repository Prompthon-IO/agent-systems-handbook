...
给初学者的默认原则是：**先用能解决问题的最小方案；当缺少结构真的开始造成痛苦时，
再引入框架。**
...
常见错误是问题还没出现就先上框架。这样会把一个小型学习项目变成依赖管理、概念学习
和调试负担。真正该问的问题不是“哪个框架最好？”，而是：
...
乐高类比仍然可以辅助理解，但不能作为唯一解释：无框架像散装积木，LlamaIndex
像专用套件，LangChain 像通用大套件，LangGraph 更像带控制逻辑的说明书，
CrewAI 像带角色分工的一组人仔。这个类比的限制也要记住：真实框架不只是“积木”，
它还定义运行时行为、状态、失败处理和系统边界。
...
| **帮你少写什么** | 不帮你省代码，所有东西自己写 | loader、索引、retriever、查询流程 | 模型/工具/提示词连接、agent harness、集成 | 状态转移、分支、持久化、可恢复执行 | 角色/任务设置和委派模式 |
...
- 如果任务足够确定性，用普通函数或工作流。不要为了显得高级而加 agent。
...
- 2026-06-02：重写为面向初学者的决策指南，更新官方链接，改进类比，并减少
  依赖具体 API 的表述。
