...
pre-filled Vision, Harness, and Craft point values. Edit those values if the
issue is more discovery-heavy, workflow-heavy, or artifact-heavy than the
starter split. The values are visible in the created issue body and a
collaborator may adjust them before approving the issue.
