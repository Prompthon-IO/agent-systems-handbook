...
Each form asks for a Prompthon repository track and work kind. Repository
...
The repository track is for path policy and routing; it is separate from the
Skill Compass point split.

Each form also includes a `Skill Compass score proposal` section with
pre-selected Vision, Harness, and Craft point dropdowns. Choose values from 0
to 100 in 5-point steps if the issue is more discovery-heavy, workflow-heavy,
or artifact-heavy than the starter split. The values are visible in the created
issue body and a collaborator may adjust them before approving the issue.
