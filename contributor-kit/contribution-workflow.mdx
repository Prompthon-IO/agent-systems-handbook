...
files into an allowed path or ask a maintainer for an explicit exception.

Use a draft PR whenever implementation, links, screenshots, or required checks
are incomplete. A non-draft PR communicates that the change is ready for
review, so mark it ready only after those items are complete.

If review uncovers substantial follow-up work, convert the PR back to a draft
while you address it, then mark it ready again when the revision is complete.
GitHub supports both stage changes from the PR page; the GitHub CLI equivalents
are `gh pr create --draft`, `gh pr ready`, and `gh pr ready --undo`. See
